...
> **TLDR:** Full-stack TypeScript starter built for AI coding agents. Server-rendered JSX (not React), custom CSS, in-memory demo data — one process, one test runner, one deploy target. Ships with CSRF protection, structured logging, and a broad test suite. The architecture is deliberately simple (services → controllers → templates) so AI agents get fast, unambiguous feedback from strict types, zero-warning linting, and tests that run in seconds. Deploy anywhere you can run `bun run start`.
>
> — *Claude Opus 4.6*
...
Security, testing, linting — these are the rails. They're solved so the agent can focus on your custom logic instead of bugging you with fundamentals.

### Security

- **CSRF protection** using HMAC tokens bound to method and path, with timing-safe comparison and origin validation
- **Environment validation** at startup — the server fails fast with clear error messages if required variables are missing
- **Structured logging** — levelled output instead of raw `console.*`

### Data

- **In-memory project list** for the CRUD demo — swap `src/server/services/project.ts` for files, an external API, or your own store when you need persistence

### Testing

- **Broad coverage** across controllers, services, middleware, utilities, and client scripts
- **No browser automation** — server-rendered templates are pure functions of props, testable with `renderToString()` and string assertions
- **Service tests** against real in-memory implementations with explicit resets between cases
- **Mock-based controller tests** that verify HTTP responses (status codes, headers, HTML content)
...
- **Separation of concerns.** Services own data, controllers orchestrate, templates render. Each layer is independently testable. An agent working on a controller works against typed service functions.
...
Open [`START_PROMPT.md`](START_PROMPT.md) with your AI coding agent. It will walk through renaming the project, stripping the Billet starter content, and verifying everything works.

Once setup is complete you can delete `START_PROMPT.md`.

### Manual setup

If you'd prefer to set up manually, install dependencies and run the dev server (optional: copy `.env.example` to `.env` if you want to override `PORT`):
...
│   ├── middleware/             # CSRF and other HTTP middleware
...
3. Deploy — Railway will build and start the server (it sets `PORT` automatically)

> **Tip:** If you're using Claude Code with the [Railway MCP server](https://docs.railway.com/guides/mcp), you can ask Claude to set up the project for you.
